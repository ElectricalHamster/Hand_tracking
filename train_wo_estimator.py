...
def train_loop():
    ...
    # features, labels = get_input(is_training=True, is_aug=False)
    '''
    while True:
        ...
        for num in range(number_img):
            # print(f"n: {num}")
            img_u8 = np.array(np.clip(img * 256, 0, 255), dtype=np.uint8)
            # cv2.imshow("", img_u8)
            # cv2.waitKey(0)
            h, w, _ = img_u8.shape
            # print(f'h:{h}, w:{w}')
    ...
    visualkeras.layered_view(feature_extractor).show()  # display using your system viewer
    visualkeras.layered_view(feature_extractor, to_file='output.png')  # write to disk
    visualkeras.layered_view(feature_extractor, to_file='output.png').show()  # write and show
    exit(0)
